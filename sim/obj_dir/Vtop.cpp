...
VL_INLINE_OPT void Vtop::_sequent__TOP__1(Vtop__Syms* __restrict vlSymsp) {
    ...
    if (VL_UNLIKELY(vlTOPp->top__DOT__trap_signal)) {
        VL_WRITEF("!!! KRITIK HATA: ISLEMCI TRAP (KILITLENME) DURUMUNDA !!!\n");
        VL_WRITEF("Son Adres: %x\n",32,vlTOPp->top__DOT__mem_addr);
        VL_FINISH_MT("top.v", 73, "");
    }
    if ((((IData)(vlTOPp->resetn) & (IData)(vlTOPp->top__DOT__mem_valid)) 
         & (~ (IData)(vlTOPp->top__DOT__mem_ready)))) {
        if (VL_UNLIKELY(vlTOPp->top__DOT__mem_instr)) {
            VL_WRITEF("KOMUT OKUNUYOR: Adres=%x\n",
                      32,vlTOPp->top__DOT__mem_addr);
        } else {
            if (VL_UNLIKELY((0U != (IData)(vlTOPp->top__DOT__mem_wstrb)))) {
                VL_WRITEF("VERI YAZILIYOR: Adres=%x Veri=%x\n",
                          32,vlTOPp->top__DOT__mem_addr,
                          32,vlTOPp->top__DOT__mem_wdata);
            }
        }
        if (VL_UNLIKELY((0x10000000U == vlTOPp->top__DOT__mem_addr))) {
            VL_WRITEF("!!! CIKTI PORTUNA YAZILDI: %10# !!!\n",
                      32,vlTOPp->top__DOT__mem_wdata);
